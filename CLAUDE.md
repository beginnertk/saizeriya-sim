...
- UIは`src/BudgetSearchPanel.tsx`に分離（`Simulator.tsx`肥大化防止のため独立コンポーネント化）。必須品・除外品の選択UIはカテゴリごとに`<details>`で折りたたみ表示（`ItemPicker`内部コンポーネント）
...
- 各結果に「この組み合わせを選択」ボタン→`qty`へ反映し`viewMode`を"menu"に戻す（`applySearchResult`関数）
  - 反映時、選んだ結果の合計額(`result.total`)を`targets.budget`（メニュー選択画面の予算上限）にも上書きする。探索中の`searchConfig.budget`とメニュー選択側の`targets.budget`は別値のまま独立管理し、この反映タイミングのみ同期する
...
7. [x] 予算探索モードの改善3点 → ①`BudgetSearchPanel.tsx`へUI分離（`Simulator.tsx`肥大化対策）
   ②反映時に`targets.budget`も選択結果の合計額へ同期 ③必須品/除外品リストをカテゴリ別`<details>`折りたたみ化
8. [ ] サイゼリヤ画像未取得8品の再調査 → TODO1参照。公式サイトの構成変更があれば再チャレンジ
9. [ ] vitest等の自動テスト基盤導入 → 現状`budgetSearch.ts`のような純関数ロジックも手動検証のみ。
   最低限`budgetSearch.ts`のgcd計算・到達可能性判定・必須/除外/カテゴリ上限のユニットテストから着手すると良い
